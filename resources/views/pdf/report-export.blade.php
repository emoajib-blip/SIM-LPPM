...
<body>
    ...
    <div class="report-type-box">
        LAPORAN KEMAJUAN {{ $proposal->detailable_type === 'App\Models\Research' ? 'PENELITIAN' : 'PENGABDIAN' }}
    ...
    <div class="section-title">1. JUDUL
        {{ $proposal->detailable_type === 'App\Models\Research' ? 'PENELITIAN' : 'PENGABDIAN' }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($proposal->teamMembers->where('pivot.role', 'anggota') as $member)
                <tr>
                    <td><strong>{{ strtoupper(format_name($member->identity?->title_prefix ?? '', $member->name, $member->identity?->title_suffix ?? '')) }}</strong><br>Anggota
                        Pelaksana</td>
    ...
    @if($report_approval_mode === 'digital' || $report_approval_mode === 'both')
        ...
        <div
            style="text-align: center; font-weight: bold; font-size: 11pt; margin-bottom: 20px; text-transform: uppercase;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 9pt;">
            <tr>
                ...
                <td width="35%">Judul
                    {{ $proposal->detailable_type === 'App\Models\Research' ? 'Penelitian' : 'Pengabdian' }}</td>
            ...
            <tr>
                ...
                <td colspan="2">Ketua {{ $proposal->detailable_type === 'App\Models\Research' ? 'Peneliti' : 'Pelaksana' }}
                </td>
            </tr>
            <tr>
                <td></td>
            ...
            @foreach($lecturerMembers as $member)
                ...
                <tr>
                    ...
                    <td colspan="2">Anggota
                        {{ $proposal->detailable_type === 'App\Models\Research' ? 'Peneliti' : 'Pelaksana' }}
                        {{ $lecturerMembers->count() > 1 ? to_roman($memberCount) : '' }}</td>
                </tr>
                <tr>
                    <td></td>
                    <td style="padding-left: 20px;">a. Nama Lengkap</td>
                    <td>{{ format_name($member->identity?->title_prefix ?? '', $member->name, $member->identity?->title_suffix ?? '') }}
                    </td>
                </tr>
                <tr>
                    <td></td>
        ...
        <table class="no-border" style="width: 100%; margin-top: 30px;">
            ...
            <tr>
                <td class="text-center" style="height: 100px; vertical-align: bottom;">
                    @if(isset($dean_signed_at) && $dean_signed_at)
                        <div style="margin-bottom: 5px;">
                            <img src="{{ generate_qr_code_data_uri('Laporan disetujui secara digital oleh: ' . $dean_name . ' (Dekan) pada ' . \Carbon\Carbon::parse($dean_signed_at)->format('d-m-Y H:i')) }}"
                                width="70">
                        </div>
                        <div style="font-size: 8pt; margin-bottom: 5px;">Disetujui pada:
                            {{ \Carbon\Carbon::parse($dean_signed_at)->format('d-m-Y H:i') }}</div>
                    @else
                ...
                <td class="text-center" style="height: 100px; vertical-align: bottom;">
                    <div style="margin-bottom: 5px;">
                        <img src="{{ generate_qr_code_data_uri('Laporan diajukan secara digital oleh: ' . $submitterFullName . ' (Ketua Pengusul) pada ' . \Carbon\Carbon::parse($lecturer_signed_at)->format('d-m-Y H:i')) }}"
                            width="70">
                    </div>
                    <div style="font-size: 8pt; margin-bottom: 5px;">Diajukan pada:
                        {{ \Carbon\Carbon::parse($lecturer_signed_at)->format('d-m-Y H:i') }}</div>
                ...
                <td colspan="2" class="text-center" style="padding-top: 20px;">
                    @if(isset($reviewer_signed_at) && $reviewer_signed_at)
                        <div style="margin-bottom: 10px; font-style: italic;">
                            Telah direview oleh Reviewer pada tanggal:
                            {{ \Carbon\Carbon::parse($reviewer_signed_at)->format('d F Y') }}
                        </div>
                    @endif
            ...
            <tr>
                <td colspan="2" class="text-center" style="height: 100px; vertical-align: bottom;">
                    @if(isset($lppm_signed_at) && $lppm_signed_at)
                        <div style="margin-bottom: 5px;">
                            <img src="{{ generate_qr_code_data_uri('Laporan diketahui secara digital oleh: ' . $lppm_head_name . ' (Kepala LPPM) pada ' . \Carbon\Carbon::parse($lppm_signed_at)->format('d-m-Y H:i')) }}"
                                width="70">
                        </div>
                        <div style="font-size: 8pt; margin-bottom: 5px;">Diperiksa pada:
                            {{ \Carbon\Carbon::parse($lppm_signed_at)->format('d-m-Y H:i') }}</div>
                    @else
                        <div style="height: 70px;"></div>
                    @endif
        ...
        <div
            style="margin-top: 60px; text-align: center; color: #666; font-size: 8pt; border: 1px dashed #ccc; padding: 15px; background-color: #fcfcfc;">
            ...
            <div style="margin-top: 5px; font-family: monospace;">ID Laporan: {{ $report->id }} | Dicetak pada:
